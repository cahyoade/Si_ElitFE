...
function TextInput({title, name, value, onChange, errorMsg, className, inputClassName}: TextInputProps) {
    return ( 
        <div className={`relative ${className}`}>
            ...
            <input type='text' value={value} name={name} autoComplete="on" className={`bg-[#f5f5f5] focus:bg-[#f9f9f9] font-bold text-lg px-4 ${value === '' ? 'py-4' : 'pt-6 pb-2'} rounded-md w-full focus:outline-none focus:border-2 focus:border-themeDarkGreen/75 focus:shadow focus:shadow-themeGreen ${inputClassName}`} onChange={ e => {
                onChange(e);
            }}/>
        </div>
    );
